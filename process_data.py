...
class Process_player_data:
    ...
    def process_strikeouts(self, total_stats):
    ...
    def process_pitcher_stats(self, total_stats, processed_pitcher_scalers=[]):
        ...
        for i in range(len(total_stats[0][3])): # Use the length of the first pitcher's stats for the loop
            ...
            if len(processed_pitcher_scalers) > 0:  # If I already have scalers, use the existing scalers. Else, use new scalers
                single_stat_scaler = processed_pitcher_scalers[i]
            ...
            if len(processed_pitcher_scalers) > 0:  # If I already have scalers, use the existing scalers. Else, use new scalers
                processed_single_stats = single_stat_scaler.transform(numpy_single_stats)
            else:
    ...
    def process_batter_stats(self, total_stats, processed_batter_scalers=[]):
        ...
        for i in range(len(total_stats[0][4])): # Use the length of the first pitcher's stats for the loop
            ...
            single_stat_scaler = MinMaxScaler(feature_range=(0,1))
            if len(processed_batter_scalers) > 0:  # If I already have scalers, use the existing scalers. Else, use new scalers
                single_stat_scaler = processed_batter_scalers[i] 
...
def main():
    ...
    # Features
    processed_pitcher_stats, all_pitcher_scalers = process_player_data.process_pitcher_stats(total_stats)
    processed_batter_stats, all_batter_scalers = process_player_data.process_batter_stats(total_stats)
